...
def test_success_get_inspection_charts_100_measurements(mocker, data_mongo_mock_100_measurements):
    # Test Request
    api_path = API_PATH.format(inspection_id=INSPECTION_ID)
    response = client.get(api_path)
    response_body = response.json()
    ...
    assert response_body.get("times")[99] == 184201
    assert response_body.get("times_formatted")[99] == "0:3:4:201"
